	...
	if($state == 'initial'){
		...
		if($intent == 'restaurantSearch'){
			...
			if($allslotsfilled){
				...
				$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/displayRestaurant?utterance=" . $user_utterance,
					"Here is a list of restaurants. Pick a restaurant from the list!");
			...
			$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/goodbye",
					"Good Bye!");
			$_SESSION['message'] = $message_temp;
			$newstate = 'finish';
		}
		else if($intent == 'greet'){
			$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/greeting",
			...
			$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/clarification",
					"I don't understand your request. Can you repeat that?");
			$_SESSION['message'] = $message_temp;
		}
		else{
			$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/clarification",
		...
		if($intent === 'restaurantSelect'){
			...
			foreach($entityArray as $entity_key => $entity_value){
				if($entity_key === 'ordinal'){
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/selectRestaurant?restaurant=" . $_SESSION['restaurant_list'][$entity_value-1]->name, 
						"You selected " . $_SESSION['restaurant_list'][$entity_value-1]->name . " on the list. What information do you want?");
					$_SESSION['message'] = $message_temp;
			...
			foreach($entityArray as $entity_key => $entity_value){
				...
				if($entity_key == 'phoneRequest'){
					...
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=phoneRequest&entity_value=" . $phone_number,
					...
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=ratingRequest&entity_value=" . $rating,
					...
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=addressRequest&entity_value=" . $address,
					...
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=reviewRequest&entity_value=" . $review,
					...
					$is_closed = $_SESSION['selected_restaurant']->is_closed;
					...
					$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=isOpenRequest&entity_value=" . $isOpenValue,
			...
			if(!$info_flag){
				$message_temp = get_system_response("http://ec2-52-4-113-236.compute-1.amazonaws.com/restaurant_info/Version1?utterance=" . $user_utterance . "&entity_name=infoError&entity_value=0",
